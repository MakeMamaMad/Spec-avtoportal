...
(function () {
  ...
  function render() {
    ...
    const filtered = allNews.filter((item, index) => {
    ...
    filtered.forEach((item, index) => {
      const id = getField(item, ['id', 'slug'], String(index));
        ...
        <div class="news-card-body">
          <header class="news-card-header">
            <h3 class="news-card-title">
              <a href="article.html?id=${encodeURIComponent(id)}">${title}</a>
            </h3>
  ...
  function handleUpdateBanner(news) {
    ...
    const first = news[0];
    const firstId = getField(first, ['id', 'slug'], '');
    const firstDate = getField(first, ['published_at', 'date', 'pub_date'], '');
    const newKey = `${firstId}|${firstDate}`;
    const prevKey = window.localStorage.getItem(STORAGE_KEY);

    // Первый заход — просто запоминаем, баннер не показываем.
    if (!prevKey) {
    ...
    // Есть обновление — считаем примерное количество новых новостей
    let diffCount = 0;
    const prevDateStr = prevKey.split('|')[1] || '';
    if (prevDateStr) {
