...
def classify(title: str, url: str = "") -> str:
    ...
    if any(k in t for k in ["цена", "подорож", "дешев", "рынок", "спрос", "продаж", "пошлин", "инфляц"]):
        return "market"
    if any(k in t for k in ["завод", "выпуск", "производств", "поставка", "дефицит", "логистик", "склад"]):
        return "supply"
    if any(k in t for k in ["тягач", "полуприцеп", "прицеп", "грузовик", "фура", "шасси", "ос", "тормоз", "подвеск", "шины", "ремонт", "сервис"]):
        return "ops"
    if any(k in t for k in ["dhl", "logistics", "перевоз", "транспорт", "контейнер", "интермодал", "терминал"]):
        return "logistics"
    return "other"
...
MEANING_BANK = {
    "rules": [
        ...
        "Что это значит: возможны новые требования. Проверь регламенты и заранее подготовь технику/документы, чтобы не ловить простой.",
    ],
    "market": [
        "Что это значит: может измениться стоимость владения. Если планируешь покупку/обновление — сравни цены и условия, закладывай запас по бюджету.",
        "Что это значит: рынок качает. Проверь, как это влияет на цену техники/запчастей и сроки поставок.",
    ],
    "supply": [
        "Что это значит: возможны сдвиги по срокам и наличию. Планируй закупки и ремонт заранее, особенно расходники.",
        "Что это значит: цепочки поставок могут меняться. Уточняй сроки у поставщиков, держи альтернативы по брендам/узлам.",
    ],
    "ops": [
        "Что это значит: это напрямую про эксплуатацию. Проверь узлы/регламент ТО, чтобы не попасть на простой в рейсе.",
        "Что это значит: можно предотвратить поломку заранее. Признаки проблемы лучше ловить до выхода на линию.",
    ],
    "logistics": [
        "Что это значит: меняются условия перевозок/маршрутов. Это может повлиять на загрузку, сроки и расходы на рейс.",
        "Что это значит: возможна перестройка логистики. Держи в уме альтернативные маршруты и окна доставки.",
    ],
    "other": [
        "Что это значит: новость смежная. Если не про коммерческий транспорт — можно пропускать.",
        "Что это значит: влияние неочевидно. Оценить стоит только если затрагивает перевозки или рынок техники.",
    ],
}
...
def pick_items(news: list[dict], used_urls: set[str]) -> list[dict]:
    ...
    title_l = extract_title(it).lower()
...
def make_digest_post(items: list[dict], slot: str) -> str:
    ...
    header = f"🚛 <b>{'Утренняя' if slot=='am' else 'Вечерняя'} сводка — {today}</b>\n<i>5 новостей + короткий вывод по каждой</i>\n"
